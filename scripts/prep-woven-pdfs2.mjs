...
async function renderFirstPage(filename, outname, dpi, maxWidth = 2000) {
  const buf = readFileSync(join(SRC, filename));
  const doc = mupdf.Document.openDocument(buf, 'application/pdf');
  const page = doc.loadPage(0);
  const bounds = page.getBounds();
  console.log(`${filename}: page bounds ${bounds[2]-bounds[0]}x${bounds[3]-bounds[1]} pt, using dpi=${dpi}`);
...
await renderFirstPage('Final Site Plan Cropped.pdf', 'proposed-site-plan', 72);
await renderFirstPage('Stepping Render.pdf', 'stepping-render', 100);
