...
### Layout
- **The site is composed at Khushi's own screen and centred beyond it (2026-09-14).** Her Dell XPS is 1536 CSS px wide (1920x1200 at 125%), about 1520px inside a maximized browser. `--frame:1520px` in `:root`; `.view.active` and `.project-page` are a three column grid (`1fr minmax(0,var(--frame)) 1fr`) with every child in the middle column, so a wider monitor shows her exact composition centred instead of each block drifting to its own left edge (at 2560 the About title sat at 60px while Experience sat at 720). Full bleed children (`.hero`, `.contact-band`, `.pp-hero`) span all three columns; their text, the nav and the hero credit use `max(60px, calc((100% - var(--frame))/2 + 60px))` to stay on the frame. `.footer` has the same max width. Two grid gotchas: a direct child with `margin:0 auto` shrinks to its content unless it also has `width:100%`, and child margins no longer collapse through a direct child. `scripts/layout-snapshot.mjs` records every element's box per page, and `--diff` proves a change left a width untouched; `scripts/responsive-audit.mjs` flags overflow, offscreen and overlapping text across 16 device widths.
- Caesura's `.pp-aside` column is reserved with `padding-right:408px` on `.pp-header` above 1024px, and stacks at 1024px and below (was 900px, which let it overlap the title on iPad landscape and small laptops). Tooltips on the tool tags are off at 768px and on touch screens: even hidden, they pushed About sideways on phones.
...
## PDF viewer
- **The resume (hero, About, contact band) and Caesura's drawing set open in a built in viewer (`#pdfv`, added 2026-09-15), not a new tab.** Modelled on the viewer Khushi designed for mann.rodeo (`1-Projects/Mann-Website`), restyled black and white: title bar, pager, Fit width / Fit page, Download, close, a 2px reading progress line, and a thumbnail rail that hides for PDFs of one or two pages. **Zoom** (2026-09-15) multiplies the current fit from 50% to 400%: minus and plus buttons with a percentage you click to reset, the + - 0 keys, trackpad pinch or ctrl + wheel over the pages, and two finger pinch on touch screens (the pages area sets touch-action so the site itself does not zoom). The point under the pointer stays put. Pages resize at once and redraw sharp 180ms after zooming pauses. Sheets centre with auto margins, not align-items:center, so a zoomed page can scroll to its left edge. At 820px and below it goes full screen with no rail, fit toggle, page arrows or percentage; just the page count, zoom buttons, download and close.
- To send another PDF to it, give the link `onclick="return openPdf(this,event)"` plus `data-pdf-title`, `data-pdf-label` and optionally `data-pdf-max` (a width cap in px; the resume uses 860). Keep the real `href`: ctrl or middle click still opens a new tab, and it is the fallback if the viewer fails.
- PDF.js 3.11.174 loads from cdnjs on first open only (3.x because it ships a plain script build; 4.x is modules only). Pages render **one at a time through a queue**, nearest the current page first, then thumbnails. Rendering everything at once is what left the heavy drawing sheets blank for 15+ seconds; sheet 3 alone takes about 3.5s to draw. Pages far off screen release their canvas to keep phone memory down.
- The viewer's Escape handler runs in the capture phase and stops propagation, so closing it does not also close the project page underneath.
- `scripts/add-pdf-viewer.mjs` inserted the first version; the render queue and resize fixes were edited into `index.html` afterwards, so treat the file as the source of truth. `scripts/shot-pdf-viewer.mjs` opens both PDFs from their real links on desktop and phone and screenshots them.
...
- **Those attributes only work alongside a bare `img{height:auto}`.** Wherever CSS sets width but not height, the browser treats the attribute height as literal pixels and the image renders at its full natural height: `117-image-trimmed.jpg` came out 327x1541 instead of 327x604. The rule is intentionally a bare `img` selector (specificity 0,0,1) so it beats the presentation attribute but loses to `.hero-img`, `.card-img`, `.model-img` and `.stack-img`, which set real heights.
- **Never replace an image in place.** Browsers keep serving the old file at the same path, so the change looks like it failed. Write a new filename instead (`-cropped.jpg`, `-trimmed.jpg`) and repoint the markup. This cost time on 070/071/075 and again on the OG image.
...
- **The preview pane sometimes stops applying programmatic scrolls.** `window.scrollTo` and `scrollIntoView` silently do nothing while wheel scrolling still works, so any test of a click handler that scrolls is meaningless in that state. Check by scrolling programmatically and reading `scrollY` back before trusting the result. Same family as the IntersectionObserver caveat above.
- **Measure layout only on settled reveals.** `.project-nav` and other `.pp-reveal` blocks sit translated 24px down until they fade in, so a spacing measurement taken too early reads 24px larger than the real gap.
