...
- **One button style for "go see the actual project work": `.we-boards-btn`** — bordered, uppercase, leading SVG icon, always with `style="position:static"` when used outside the aside (the base rule is absolutely positioned). Every such trigger on the site uses it; there are no `.dl-link` flipbook triggers left. `.dl-link` now means secondary only — currently just the resume link.
  - **Top right** is the default placement: in the `.pp-aside` slot of `.pp-header`, which then needs `style="position:relative;max-width:none"` with its content wrapped in `<div style="max-width:900px">`. Used by Caesura, Woven Edge, Fluke, Dreamscape, Lakër, Drodel. Below 900px the aside goes static and the button stacks above the title.
  - **Centred under the write-up** where a second, subordinate link has to hang off it. Beacon does this: button centred below `.pp-description`, then the site analysis `.model-links` link directly beneath it (`justify-content:center;padding:18px 60px 60px`), the 60px keeping it clear of the `.project-nav` rule. The additional-work pages (both model pages, Virtuous Book, Beaufort Cookbook, Site Analysis) are centred in place too.
