...
- Nav is Home / Work / About in a fixed top bar, present on every view. (There is no `body.in-hero` behaviour: that was described here for months but never existed in the file.) On mobile the top bar carries only the wordmark, and a fixed bottom tab bar carries work / about / contact.
